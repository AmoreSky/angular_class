...
export class ADMINDASHBOARD implements OnInit {
  onNav(id: string) {
  const item = this.adminNavItems.find(i => i.id === id);
  if (item && (item as any).routerLink) {
    this._router.navigate([(item as any).routerLink]);
    return;
  }
  // fallback mapping (if you used labels as ids)
  if (id === 'Add Admin' || id === 'add-admin') {
    this._router.navigate(['/add-admin']);
  }
  // add additional id -> route mappings as needed
}
  ...
  adminNavItems: NavItem[] = [
    {
      ...
      routerLink: '/admin-dashboard/add-admin'
    },
    {
